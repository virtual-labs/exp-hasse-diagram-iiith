...
function enableEdgeEvents() {
  if (this.options.noEdgeEventsInDraw) {
    ...
    var src= this.cy.edges()[this.cy.edges().length -1 ].data().source.toString();
    var tgt= this.cy.edges()[this.cy.edges().length -1 ].data().target.toString();
    var edg = src+'-'+tgt;
    console.log(edg)
    //console.log(this.cy.edges()[this.cy.edges().length -1 ].data().source.toString() +'-'+this.cy.edges()[this.cy.edges().length -1 ].data().target.toString())
    if (!edgedata.includes(edg)) {
        edgedata.push(edg);
      }
